import { Trip, WeatherLog, FishCaught, DB_CONFIG, DatabaseError } from '../types';
...
export class DatabaseService {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(DB_CONFIG.STORES.WEATHER_LOGS)) {
          const weatherStore = db.createObjectStore(DB_CONFIG.STORES.WEATHER_LOGS, { 
            keyPath: 'id', 
            autoIncrement: true 
          });
      ...
      request.onerror = (event) => {
        ...
        reject(this.createDatabaseError('connection', `Failed to initialize database: ${error?.message}`));
      };
    });
  ...
  getDatabase(): IDBDatabase {
    if (!this.db) {
      throw this.createDatabaseError('connection', 'Database not initialized. Call initialize() first.');
    }
    return this.db;
  }
  ...
  async createTrip(tripData: Omit<Trip, 'id'>): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.TRIPS], 'readwrite');
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to create trip: ${request.error?.message}`);
        console.error('Error creating trip:', error);
        reject(error);
      };

      transaction.onerror = () => {
        const error = this.createDatabaseError('transaction', `Transaction failed: ${transaction.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getTripById(id: number): Promise<Trip | null> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.TRIPS], 'readonly');
      const store = transaction.objectStore(DB_CONFIG.STORES.TRIPS);
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get trip: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getTripsByDate(date: string): Promise<Trip[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get trips by date: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getAllTrips(): Promise<Trip[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get all trips: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async updateTrip(trip: Trip): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to update trip: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async deleteTrip(id: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to delete trip: ${transaction.error?.message}`);
        reject(error);
      };

      // Delete associated weather logs
      const weatherStore = transaction.objectStore(DB_CONFIG.STORES.WEATHER_LOGS);
  ...
  async createWeatherLog(weatherData: Omit<WeatherLog, 'id'>): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.WEATHER_LOGS], 'readwrite');
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to create weather log: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getWeatherLogById(id: number): Promise<WeatherLog | null> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.WEATHER_LOGS], 'readonly');
      const store = transaction.objectStore(DB_CONFIG.STORES.WEATHER_LOGS);
      const request = store.get(id);
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get weather log: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getWeatherLogsByTripId(tripId: number): Promise<WeatherLog[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get weather logs by trip ID: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getAllWeatherLogs(): Promise<WeatherLog[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get all weather logs: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async updateWeatherLog(weatherLog: WeatherLog): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to update weather log: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async deleteWeatherLog(id: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to delete weather log: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async createFishCaught(fishData: Omit<FishCaught, 'id'>): Promise<number> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.FISH_CAUGHT], 'readwrite');
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to create fish caught record: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getFishCaughtById(id: number): Promise<FishCaught | null> {
    ...
    return new Promise((resolve, reject) => {
      const transaction = this.getDatabase().transaction([DB_CONFIG.STORES.FISH_CAUGHT], 'readonly');
      const store = transaction.objectStore(DB_CONFIG.STORES.FISH_CAUGHT);
      const request = store.get(id);
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get fish caught record: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getFishCaughtByTripId(tripId: number): Promise<FishCaught[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get fish caught records by trip ID: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async getAllFishCaught(): Promise<FishCaught[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to get all fish caught records: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async updateFishCaught(fishCaught: FishCaught): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to update fish caught record: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async deleteFishCaught(id: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to delete fish caught record: ${request.error?.message}`);
        reject(error);
      };
    });
  }
  ...
  async clearAllData(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () => {
        const error = this.createDatabaseError('transaction', `Failed to clear data: ${transaction.error?.message}`);
        reject(error);
      };
   ...
   */
  private createDatabaseError(type: DatabaseError['type'], message: string): DatabaseError {
    return {
      type,
      message,

...
describe('GalleryModal', () => {
  ...
  it('opens full-size photo modal when photo is clicked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('button', { name: /close/i }) || 
             document.querySelector('.fa-times')).toBeInTheDocument();
    });
  });
  ...
  it('shows empty state when no photos exist', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Add photos to your fish catches to see them here')).toBeInTheDocument();
    });
  });
  ...
  it('shows filtered empty state when no photos match filter', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('No photos found')).toBeInTheDocument();
      expect(screen.getByText('Try selecting a different month or clear the filter')).toBeInTheDocument();
    });
  });

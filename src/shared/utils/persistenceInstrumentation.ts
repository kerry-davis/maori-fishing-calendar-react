...
export class BrowserAPISafe {
   ...
   */
  static safeLocalStorage(key: string, operation: 'get' | 'set' | 'remove' | 'clear', value?: unknown): any {
    try {
      if (typeof localStorage === 'undefined') {
   ...
   */
  static safeSessionStorage(key: string, operation: 'get' | 'set' | 'remove' | 'clear', value?: unknown): any {
    try {
      if (typeof sessionStorage === 'undefined') {
